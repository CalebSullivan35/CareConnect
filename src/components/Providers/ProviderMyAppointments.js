...
export const ProviderMyAppointments = () => {
  ...
  const [allProviderAppointments, setAllProviderAppointmenets] = useState([]);
  const [scheduledProviderAppointments, setScheduledProviderAppointments] = useState([])
  ...
  useEffect(() => {
    fetch(`http://localhost:8088/providerAllAppointments?providerid=${currentProvider.id}`)
    .then((response) => response.json())
    .then((data) =>{
        setAllProviderAppointmenets(data)
    });
  },[currentProvider]);
  ...
  return (
    <div className="flex flex-row">
      ...
      {showCreateAppointment ? <CreateNewAppointment /> : ""}
      <button
